...
class EmbeddingDriftAnalyzer(BaseAnalyzer):
    ...
    def analyze(self, observations: Dict[str, Any]) -> List[AnalysisResult]:
        ...
        for param_name, obs_list in parameters.items():
            ...
            # Heuristic: 2D matrices where param name contains 'embed' or vocab > dim
            is_2d = len(shape) == 2
            name_suggests_embedding = "embed" in param_name.lower()
            shape_suggests_embedding = is_2d and shape[0] > shape[1]

            if not (is_2d and (name_suggests_embedding or shape_suggests_embedding)):
                continue
